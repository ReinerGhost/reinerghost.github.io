(function () {
    ...
    function getFormData() {
        let formDataTmp = new FormData(document.querySelector('form.wash-fold-form-first-step'));
/*        console.log(Array.from(formDataTmp));
    ...
    function summationPrice() {
        ...
        if ($('.wash-fold-form-first-step [name="bleach"]').val() == 'YES') Price = Price + parseInt(settings.wash_bleach_whites_price);
        if ($('.wash-fold-form-first-step [name="extra_bag"]').val() == 'YES') Price = Price + parseInt(settings.wash_laundry_bag_price);
        $('.sum-body').html(Price);
    }
...
/*        data.push({
    ...
    function getWashData(){
        $.ajax({
            type: "GET",
            url: helper.app.HomeSite + '/order-supplies-mobile',
            dataType: 'json',
            success: function (data) {
                localStorage.setItem('lastOrderSuppliesData', JSON.stringify(data));
            }
        });
    }

    washFold = new Page('washFold', function (name) {
        // console.log(name);
        var self = this;
        ...
        self.render({ 'Settings': settings, 'User': getUserData() }, function () {

            let lastOrderSuppliesData = localStorage.getItem('lastOrderSuppliesData');
            if (lastOrderSuppliesData != null){
                lastOrderSuppliesData = JSON.parse(lastOrderSuppliesData);
                for (let key in lastOrderSuppliesData){
                    $('select[name=' + key + '] option[data-supplyname="' + lastOrderSuppliesData[key] +'"]').prop('selected', true);
                }
            }

            //detergent
            $('.wash-fold-form-first-step [name="detergent"]').change(styledSelector);
            styledSelector.call($('.wash-fold-form-first-step [name="detergent"]')[0]);
            //softener
            $('.wash-fold-form-first-step [name="softener"]').change(styledSelector);
            styledSelector.call($('.wash-fold-form-first-step [name="softener"]')[0]);
            //bleach
            $('.wash-fold-form-first-step [name="bleach"]').change(styledSelector);
            styledSelector.call($('.wash-fold-form-first-step [name="bleach"]')[0]);
            //extra bag
            $('.wash-fold-form-first-step [name="extra_bag"]').change(styledSelector);
            styledSelector.call($('.wash-fold-form-first-step [name="extra_bag"]')[0]);
            //tips numbers
            ...
            $('button.second-step').click(function () {
                try {
                    ...
                    globalData['data'] = cache; globalData['textData'] = getDataText(cache); globalData['title'] = 'WASH &amp; FOLD'; globalData['goBack'] = 'washFold';
                    $('.loading-block').show();
